...
arima_prediction <- function(training, test, ar, ma, d){
  #kell egy üres data.frame, ami majd a végsõ eredményeket tárolja
  info <- data.frame()
  
  test <- diff(test, differences = d)
  ...
  for (phi in 0:ar)
  {
    for (theta in 0:ma)
    {
      ...
      hiba <- mse(actual = test[(d+1):length(test)], predicted = teljes[(length(teljes) - length(test) + 1 + d):length(teljes)])
